...
parser.add_argument('--weight_decay', type=float, default=2e-4,
                    help='The weight decay of l2 regularization.')

parser.add_argument('--keep_prob', type=float, default=0.5,
                    help='Dropout keep prob.')

parser.add_argument('--is_predict', type=bool, default=False,
                    help='is do prediction')

parser.add_argument(
    '--network', type=str, default="resnet",
    choices=['imagenet', 'resnet', 'resnet_dropout', 'resnet_bottleneck'],
    help="Network type")
...
def get_filenames(is_training, data_dir):
    ...
    if is_training:
        return [
            os.path.join(data_dir, 'train_1311_3.bin')
        ]
    else:
        return [os.path.join(data_dir, 'val_1311_3_no_label.bin')]
...
def parse_record(raw_record):
    ...
    # and then to one-hot.
    label = tf.cast(record_vector[0], tf.int32) * 100 + tf.cast(record_vector[1], tf.int32)
...
def cifar10_model_fn(features, labels, mode, params):
    ...
    if FLAGS.network == 'imagenet':
        ...
        network = resnet_model.cifar10_resnet_dropout_generator(
            params['resnet_size'], _NUM_CLASSES, params['data_format'], keep_prob=FLAGS.keep_prob)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        boundaries = [int(batches_per_epoch * epoch) for epoch in [60, 100, 150, 170]]
...
def main(unused_argv):
    ...
    if FLAGS.is_predict:
        predictions = cifar_classifier.predict(
            input_fn=lambda: input_fn(False, FLAGS.data_dir, FLAGS.batch_size)
        )
        class_prediction = [predict['classes'] for predict in list(predictions)]
        print(class_prediction)
        np.savetxt('final.txt', np.array(class_prediction), fmt="%d", newline=' ')
