...
def cifar10_resnet_dropout_generator(resnet_size, num_classes, data_format=None, keep_prob=0.5):
    """Generator for CIFAR-10 ResNet v2 models.
    ...
    def model(inputs, is_training):
        ...
        inputs = tf.layers.dropout(inputs, rate=keep_prob, training=is_training)
        inputs = conv2d_fixed_padding(
            inputs=inputs, filters=16, kernel_size=3, strides=1,
            data_format=data_format)
        inputs = tf.identity(inputs, 'initial_conv')
...
def cifar10_resnet_bottleneck_generator(resnet_size, num_classes, data_format=None):
    """Generator for CIFAR-10 ResNet v2 models.
